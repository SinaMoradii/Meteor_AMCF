...
typedef enum {
   ...
   SIG_PDINTERVAL = 0x0000000A,   // Specify the interval for a repeating product detect generated internally by the FPGA.
                                  // The repeating product detect can be run continuously, or triggered from an external PD / force PD, in which case N product detects
                                  // are generated for each trigger according to the value set by <see cref="F:Ttp.Meteor.SigTypes.SIG_PD_REPEAT" />
                                  ...
                                  // Advanced product detect sequences can be set up by using <see cref="F:Ttp.Meteor.SigExTypes.SIGEX_PRODUCT_DETECT_QUEUE" />
   ...
   SIG_SPIT = 0x0000000B,   // Do a number of spits (fire on every nozzle) on the specified HDC. It is also possible to spit on all HDCs, see parameters.
                            ...
                            // <i>xaddr </i>
                            // 0:     use the current printing waveform without reloading the waveform to the hardware.
                            // 255:   force a reload of the current waveform to the hardware before the spit.
                            // 1-100: temporary waveform id for the spit.  This waveform is loaded to the hardware before the spit, and the original waveform is restored 
                            // during the next StartJob (or in a subsequent spit with xaadr 255).
                            // A non-zero value cannot be used during a print job.
                            // Currently supported for Xaar head types only.
   ...
   SIG_ENABLEPD = 0x0000000F,   // Enable/Disable internal and external PDs.
                                // When SIGEX_PRODUCT_DETECT_QUEUE is being used, disabling internal PDs prevents a running internal PD queue from generating 
                                // triggers, but the queue continues to run (so the intervals are maintained).  
                                // bmSigExPd_StopInternalPdQueue and bmSigExPd_StartInternalPdQueue can be used to stop and restart the internal PD queue itself.
                                // <i>xaddr </i>Normally 0.  Set to 1 for an additional mode where state:Bit0 completely disables external PD triggers.
                                // This can be used alongside a software triggered internal PD queue to protect against spurious external triggers.
   ...
   SIG_STEPPER = 0x00000016,   // Start GPIO-controlled stepper drive movement
   ...
   SIG_SKIP_DOC = 0x00000028,   // Skip FIFO document in PCC memory based on its 24-bit DocId (Cmd[2] in PCMD_STARTFDOC).
                                // There are two skip document modes:
                                // --- MODE 0 (Bit30 clear): Discard the document completely when a PD is received and print the next document.
                                // ---                       The document ID for the skipped document is not generated.
                                // ---                       i.e. this mode assumes that a PD is not received for a document which needs to be skipped.
                                // --- MODE 1 (Bit30 set):   Process the start document command when a PD is received but discard all image data.
                                // ---                       A document ID is generated for the skipped document.
                                // ---                       i.e. this mode assumes that a PD is received for a document which needs to be skipped.
                                // A queue of up to 256 IDs to skip is maintained in the PCC.  Note that the IDs to skip must be sent in the same order as the
                                // original documents.
                                // The PCC's skip document queue is reset during the start job sequence.
   ...
   SIG_PD_REPEAT = 0x0000002A,   // Specify the product detect repeat count: this is the *total* number of product detects that are generated from a single trigger
                                 // (external PD signal, or force PD).  Overrides the config file's [ProductDetect] Repeat parameter, if set.
   ...
   SIG_STATUS_UPDATE_MS = 0x0000002F,   // Set the target rate at which the PCC sends status updates to the PC.  The rate is updated in the hardware immediately.
                                        // Default is 300ms for current hardware platforms.  A rate of less than 50ms is not recommended.
                                        // Can also be set in the configuration file using [Test] PcceStatusTimerMs.
   SIGTYPE_COUNT = 0x00000030,   // Count of valid SigTypes
} SigTypes;
...
typedef enum {
   ...
   HT_KJ4 = 0x00000004,   // Kyocera KJ4, being deprecated. Use HT_KY0600_KA instead
   ...
   HT_Xaar_FS = 0x0000004B,   // Xaar_FS project
   HT_SF600 = 0x0000004C,   // Dimatix SF600 head
   HT_PJU_80 = 0x0000004D,   // PJ unit with 80 "nozzles"
   HT_Xaar502 = 0x0000004E,   // Xaar502 1-head HDC
   HT_SEIKO_RCE2560 = 0x0000004F,   // Seiko RCE2560 head
   HT_KY0600_KA = 0x00000050,   // New software design for former KJ4 head. (obsoletes HT_KJ4)
   HT_COUNT = 0x00000051,   // Count of HT_xxx values
} eHEADTYPE;
...
#define BMPS_HDPWR_WDT_EXPIRED 0x00000008   // This status flag indicates that PCC head power Watchdog Timer has expired and all HDCs have been powered off.
#define BMPS_IPBUSY 0x00000010   // Receiving print data from USB
#define BMPS_DDFULL_obsolete 0x00000020   // DDRAM is full (obsolete: USB only)
#define BMPS_SPURIOUSPD 0x00000040   // A spurious product detect error is flagged if a PD is detected during the lockout period
...
#define BMPS_PCC_STATE 0xF0000000   // Obsolete: this field was used for USB platforms only.  [4-bit Print Controller state field (PS_xxx)]
...
#define SH_PCC_STATE 0x0000001C   // Obsolete: this field was used for USB platforms only.  [Right-Shift Value for aligning the field at the lsb of PCC State]
...
typedef enum {
   EFOCtrl_SetHDC = 0x00000000,   // Set HDC[1..8] for FireOut
   EFOCtrl_SetExpOffDelay = 0x00000001,   // Number of fire pulses to hold the exposure high for after the fire burst has finished
   EFOCtrl_SetExposureMode = 0x00000002,   // Selects the exposure trigger mode
   EFOCtrl_SetSyncMode = 0x00000003,   // Syncs the first fire pulse to the rising edge of encoder A
   EFOCtrl_SetFireBurstLen = 0x00000004,   // Configures the number of fire pulse to produce. 0 is unlimited
   EFOCtrl_SetFireOffset = 0x00000005,   // Configures the line in the image in which to start producing fire pulses.
   EFOCtrl_SetPulse1_Delay_ps = 0x00000006,   // Set FireOut Pulse1 delay in picoseconds
   EFOCtrl_SetPulse2_Delay_ps = 0x00000007,   // Set FireOut Pulse2 delay in picoseconds
   EFOCtrl_SetPulse_Width_ps = 0x00000008,   // Set FireOut Pulse width in picoseconds
} TFireOutControl;
...
#define JT_FLAG_RIGHTALIGN 0x40000000   // Flag can be set in the <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_STARTJOB" /> Job Type field (Cmd[3]) to change the meaning of the 
...
#define DATAPATH_OVERRIDE 0x40000000   // Bit set as part of the <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_CF_HDIMAGE_BLK" /> command to override the dual FIFO datapath for the image for JT_DUAL_FIFO.
...
#define MAX_SUBSYS 6   // The maximum number of sub-systems which can be driven by one PC running the PrintEngine.
...
#define BM_CFG_RELOAD_PLANE_POSITIONS 1024   // Set this bit (BIT10) in PiReloadConfigValues to reload the plane position parameters e.g. [Planes] YCropPlaneN from the config file
...
#define BM_SAVE_SIM_FILES 0x00200000   // Is saving .sim files currently enabled.
...
#define HDWS_CROSSCHECK_FAILED 0x00000007   // WF crosscheck failed (for heads that have multiple WFs)
...
typedef enum {
   ...
   PCMD_IMAGE = 0x7A5535A4,   // SendImage command id
                              ...
                              // Cmd[4] = YTop (pixels)
                              // Cmd[5] = Width (pixels)
                              // Cmd[6-N] = Image data.  Sent line by line.  The size of each line (the buffer stride) must be a whole number of DWORDS, any additional
                              // data past the pixel width is ignored.  The most significant bit(s) of each DWORD contain the left-most pixel.  See example below.
                              // e.g. to send a 1 pixel (x) by 2 pixel (y) image at 1bpp, with both pixels set, positioned at (100,200) in plane 1 in the document, the command data is:
                              // {PCMD_IMAGE, 6, 1, 100, 200, 1, 0x80000000, 0x80000000};
                              // This is designed to print streamed image data. To print a file from disk use <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_TIFFIMAGE" /> or <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_IMAGE_BUFFER" />.
   ...
   PCMD_HDIMAGE = 0x7A5535A5,   // SendHeadDriverCardImage command id, used to send image data to a single Head Driver Card, bypassing the Meteor image translator
                                ...
                                // Cmd[2] = Bits[7:0]: PCC number (1-pcc count)
                                // Cmd[3] = Bits[7:0]: Head Driver Card number (1-8)
                                // Cmd[4] = Bits[23:0]: XStart (pixels)
                                // Cmd[5-N] = Head image data
                                // To send pre-compressed RLE data, set BIT31 of Cmd[2].  The Command DWORD count becomes (4 + compressed DWORDs) and Cmd[5] is used for
                                // the (uncompressed) image X length.  The compressed data starts at Cmd[6] instead of Cmd[5].
   ...
   PCMD_TIFFIMAGE = 0x7A5535AA,   // TiffImage command id
                                  ...
                                  // Bit31 Set to start a parallel asynchronous load of the TIFF file (no print commands are sent; a second PCMD_TIFFIMAGE command 
                                  ...
                                  // The PiSendTiffImageCommandHelper function can be used to format the filename pointer and handle string marshalling in managed code.
                                  // Load .tif files asynchronously in parallel to increase throughput speed. See C:\Program Files\Meteor Inkjet\Meteor\Samples\SampleSinglePassPrint.zip for an example.
                                  // N.B. When a print job resolution divider is in use, the x position is at the *undivided* resolution (the config file's base 
                                  // resolution) for backwards compatibility.  When CCP_XRES_SCALE is used to change resolution, the x position is at the 
                                  // print resolution.
   ...
   PCMD_REPEAT = 0x7A5535AC,   // Repeat command id, for seamlessly repeating an image when using the preload path.
                               ...
                               // A repeat of N means that N copies of the image will be printed.  The maximum number of repeats is 0xFFFFFF (16,777,215).
                               // Cmd[0] = PCMD_REPEAT
                               // Cmd[1] = 1 (DWORD count)
                               ...
                               // In this mode, just a single circular copy of the image is written to PCC RAM.  It can also be selected using [Test] BigImage = 1.
   ...
   PCMD_BIGIMAGE = 0x7A5535B4,   // SendBigImage command id
                                 // This can be used to stream very big images that will not fit an a reasonable sized command buffer. It sends the image to the hardware in sections.
                                 // It's recommended to first try using <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_IMAGE" /> with the config setting [System] CmdBufSizeDwords to increase the buffer size.
                                 // Alternatively the image can be split into sections and printed seamlessly together using <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_SET_CONTINUOUS_PRINT" />. 
                                 // This allows the application full control of the queueing.
                                 // To load large files use <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_TIFFIMAGE" />.
                                 // Only use PCMD_BIGIMAGE if the command buffer would consume too much memory on the system AND it's required to be streamed.
   ...
   PCMD_PRINT_RAM_IMAGE = 0x7A5535BA,   // Print Image Stored in PCC RAM
                                        ...
                                        // Cmd[1] = Command DWORD count: 3 for standard printing, 4 for circular printing, 5 to target a single PCC, 7 for continuous/seamless copies
                                        // Cmd[2] = Image Reference Number, 0 - unsigned 32-bit integer (the value which was sent to a previous PCMD_WRITE_RAM_IMAGE_EX command)
                                        ...
                                        // Cmd[7] = [Optional] Number of seamless copies of the image.  0 for infinite repeats.  Default is 1 (a single copy of the image)
                                        // Cmd[8] = [Optional] X gap between continuous copies.  Default is 0.
   ...
   PCMD_IMAGE_BUFFER = 0x7A5535BC,   // Send an image from an image buffer previously allocated and filled by PiAllocateTiffImageBuffer, or allocated by 
                                     ...
                                     // 
                                     // e.g. to free a buffer asynchronously without printing it, use the command DWORDS:
                                     // PCMD_IMAGE_BUFFER | 5 | 0x80000000 | 0 | 0 | 0 | imageBufId
                                     // Where the top bit of cmd[2] is set to free the buffer, and the plane is zero so the buffer isn't printed.
   ...
   PCMD_SET_CONTINUOUS_PRINT = 0x7A5535C2,   // Similar to CCP_SET_CONTINUOUS_PRINT, except switches continuous mode on or off synchronously with the print queue
                                             // Can be used to switch a running FIFO print job between continuous and non-continuous mode
   ...
   PCMD_RESERVED6 = 0x7A5535C4,   // Reserved for future use
   PCMD_RIPIMAGE = 0x7A5535C5,   // Rip-and-print image command id
                                 ...
                                 // Cmd[15] = The number of Scalable RIPs that is used.
                                 // Cmd[16] = The amount of memory that is divided evenly between all RIP instances.
                                 // Cmd[17] = The ScreenPro Screen Mode Index, if the output of the RIP is 8 or 16 BPP. 0 selects the default screen from the config file.
                                 // Cmd[18-34] = Reserved for future use.  Must be zero.
   PCMD_ROTATE = 0x7A5535C6,   // Quickly rotate an image in range of [-45, +45] degrees immediately before translating.
                               ...
                               // Images with a large aspect ratio cannot be rotated as much, a 4:1 image can only be rotated by ~[-14,+14] degrees. Limitation for this is
                               ...
                               // The required time can be estimated using the Excel spreadsheet attached to the application note.
                               ...
                               // Cmd[2] = Signed 32-bit integer describing rotation angle in 10^-7 degree steps, i.e. rotation angle = Cmd[2]/10^7. Send an angle >45 degrees to skip this print.
   ...
   PCMD_NOZZLECOMP_OVERRIDE = 0x7A5535C8,   // Override the current nozzle compensation mode for the next document only.
   ...
   PCMD_FASTMASK = 0x7A5535CD,   // Mask an image using a set of 4 coordinates immediately before translating.
                                 ...
                                 // These requirements are very similar to the <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_ROTATE" /> command. Therefore it's recommended to also read the Image Rotation Application Note.
                                 // The required time can be estimated using the Excel spreadsheet attached to the application note.
   ...
   PCMD_UPDATE_TRANSLATOR_PARAMS = 0x7A5535CE,   // Request translator to update some parameters on-the-fly during a print job.
                                                 // Load new parameter values using PiReloadConfigValues or PiReloadParamEx, then call this to use them in the current job.
                                                 // Cmd[0] = PCMD_UPDATE_TRANSLATOR_PARAMS
                                                 // Cmd[1] = 2 (command DWORD count)
                                                 // Cmd[2] = Set one or more <see cref="T:Ttp.Meteor.UpdateTranslatorFlags" /> bits to reload specific parameters
                                                 // Cmd[3] = Reserved for future use
   PCMD_ADJUST_COLOUR_BALANCE = 0x7A5535CF,   // Update colour balance values while in a job. Can optionally specify grey levels and regions of the image. It will apply different colour balance values to each grey level and/or region.
                                              // This command reloads colour balance values set in the config file, then overrides them with values set in this command. Any values from a previous call of this command are lost.
                                              // On start job the config file values will be reloaded. This command must be called during a job to overwrite the config values.
                                              // See the Adjust Colour Balance Application Note for examples and explanation of the string formatting.
                                              // See PrinterInterfaceCLS.PiAdjustColourBalanceCommand for C# (or just an example of sending the command).
                                              // Cmd[0] = PCMD_ADJUST_COLOUR_BALANCE
                                              // Cmd[1] = X (total command DWORD count)
                                              // Cmd[2] = Colour plane to update (1-<see cref="F:Ttp.Meteor.MeteorConsts.MAX_PLANES">MAX_PLANES</see>).
                                              // Cmd[3] = Number of regions to specify (can be 0). Each region is a rectangle with its own colour balance values that override the global values.
                                              // Cmd[4-N] = Specify each region in the format (x, y, width, height) (in px). X and Y are relative to the input image. N = (3 + NumRegions * 4).
                                              // For example to specify regions (x1,y1,w1,w2), (x2,y2,w2,h2): Cmd[3] = x1, Cmd[4] = y1, Cmd[5] = w1, Cmd[6] = h1, Cmd[7] = x2, Cmd[8] = y2, Cmd[9] = w2, Cmd[10] = h2.
                                              // Cmd[N-X] = A list of colour balance settings for each given (region, grey level) combination. Each section should look like the following:
                                              // SubCmd[0] = [BIT7:0] Grey Level to set (0 for all grey levels); [BIT15:8] Region to set (0 to set global colour balance, [1-NumRegions] to specify each region defined above).
                                              // SubCmd[1] = Length of colour balance string in BYTEs.
                                              // SubCmd[2-(StrLenDwords+2)] = The colour balance string to set as a char array copied in place. Add 0 padding up to the next DWORD if required. NULL terminator is not required.
                                              // Place each SubCmd into the Cmd sequentially. As many entries can be added as required.
} CtrlCmdIds;
...
typedef enum {
   ...
   CCP_HEAD_VOLTAGE__Obsolete = 0x00000005,   // Obsolete.  See technical notes for details about how to set head voltages for a particular head type.
   ...
   CCP_PRINT_CLOCK_HZ = 0x0000001B,   // Master print clock frequency (internal encoder, Hz).  Zero for external encoder.  Updates either during the next
                                      // start job, or if the <see cref="F:Ttp.Meteor.SigTypes.SIG_UPDATEINTERNALENCODER" /> signal is set.
   ...
   CCP_STATUS_UPDATE_MS = 0x0000002C,   // Set the sleep interval which the PC uses when polling for PCC status.  The default, and maximum, is 10ms.  
                                        // Larger values are ignored for all Ethernet platforms.  CCP_STATUS_UPDATE_MS is maintained mainly for backwards compatibility and should 
                                        // not normally be used for new applications.  A value of 0ms is still allowed, which can sometimes give a marginally quicker status
                                        // polling cycle when used alongside SIG_STATUS_UPDATE_MS.
   CCP_SET_CONTINUOUS_PRINT = 0x0000002D,   // Enable/disable FIFO continuous print mode.  Can also be used with PRELOAD and PCMD_REPEAT for a continuous sequence 
                                            // of N(1) seamless repeats of image 1, followed by N(2) seamless repeats of image 2 etc.
   ...
   CCP_HEAD_TEMP_MAX__Obsolete = 0x00000038,   // Obsolete.
   CCP_HEAD_PALETTE__Obsolete = 0x00000039,   // Obsolete.  See technical notes for details of head types that support palettes.
   CCP_DRIVER_TEMP_MAX__Obsolete = 0x0000003A,   // Obsolete.
   ...
   CCP_MEM_BOUNDARY = 0x0000004A,   // Boundary dword address between datapaths within HDC/head memory on the PCC.  The value is set in DWORDs and
                                    // takes effect when the next print job is started.  See MixedMode.txt in the technical notes for more information.
   ...
   CCP_TRANSLATOR_MODE = 0x0000004E,   // BIT0 set: disable fast translator;  BIT1 set: disable SIMD translator; BIT2 set: disable parallel translator; BIT3 unused; BIT4 set: disable extended SIMD print line
   ...
   CCP_PCCX_TICKLEDIVIDER__Obsolete = 0x00000054,   // Obsolete.
   ...
   CCP_SET_BRUSH_MODE = 0x00000063,   // enable (1 to enable, 0 to disable), will cause print trigger to set PD/advance when trigger goes low, so that a seamless repeated image loop will be ended
...
typedef enum {
   ...
   CPEX_TargetTemp = 0x00000116,   // Target temperature
                                   // Head Types: All currently supported head types where temperature control is available.
   ...
   CPEX_FlagData_FileIdx = 0x00000147,   // head flag data file index
   ...
   CPEX_Palette0 = 0x00000155,   // palette word0. Used by heads with true 4BPP.
   CPEX_Palette1 = 0x00000156,   // palette word1. Used by heads with true 4BPP.
   ...
   CPEX_Spare1 = 0x00000161,   // Used to be KPEX_AsicStitch_FileIdx, it is deprecated. Spare now
   ...
   CPEX_AuxRowX_Adj_cpix = 0x00000174,   // Experimental X-offset adjust in 1/100 of a pixel
   CPEX_OnTheFly_HdcXAdjust_cpix = 0x00000176,   // Offset which can be applied to the whole HDC X position during a print job to adjust the X position on the fly.
                                                 // The offset is in centi-pixels and is always applied at the printing resolution.
                                                 // Prior to being applied, this 1/100 pixel resolution is rounded to ticks (i.e. 1/15 or 1/16 pixel, depending on the head type).
                                                 // The offset can be set globally, per PCC, or per HDC.
                                                 // The updated registers are written when the application calls PiUpdateHdcXoffsets.
   CPEX_Clock2 = 0x00000177,   // Clock2 value. Mostly applicable for HT_KJ4
   CPEX_ForceABC_Mode = 0x00000178,   // Force ABC mode for some of Xaar heads
} eCFGPARAMEx;
...
typedef enum {
   ...
   KSP_DisabledNozzles = 0x00000013,   // string Pool Id for storing lists of nozzles disabled on a particular color plane. StrIdx is 0-indexed plane.
   ...
   KSP_FlagData_FN = 0x00000016,   // string Pool Id for head "flag data" files
   ...
   KSP_AdjustColourBalance = 0x00000019,   // string Pool Id for Adjust Colour Balance settings. StrIdx is 0-indexed plane, or grey level * <seealso cref="F:Ttp.Meteor.MeteorConsts.MAX_PLANES" /> + plane (both 0-indexed) for per grey level adjustments.
   ...
   KSP_Spare1 = 0x0000001D,   // used to be KSP_AsicStitch_FN, but is was deprecated. Spare now.
...
typedef enum {
   ...
   RVAL_BADSIZE = 0x00000028,   // Incorrect size (e.g. the size of a file on disk does not match with its file header details)
   RVAL_INPUT_EXECPTION = 0x00000029,   // A fatal PrintEngine exception occurred during input command processing.  If a crash dump was generated, please
                                        // email a status report to Meteor support.
} eRET;
...
typedef enum {
   ...
   MPS_CONNECTED = 0x00000001,   // Obsolete (Was 'Connected to PC' on USB hardware)
   MPS_IDLE = 0x00000002,   // Waiting for print data
   MPS_READY = 0x00000003,   // Have print data, waiting for trigger.
                             // The "ready" state covers a large number of different situations.  To check whether a particular document is fully ready to print, an application
                             // should normally use the job documents queued counters in the <see cref="T:Ttp.Meteor.TAppStatus" /> structure (e.g. <see cref="F:Ttp.Meteor.TAppStatus.DocsQueuedLane1" /> etc.).
   MPS_PRINTING = 0x00000004,   // At least one HDC is printing.
   MPS_INITIALIZING = 0x00000005,   // Configuring FPGA
   MPS_STARTUP = 0x00000006,   // A printer status of MPS_STARTUP is never set by the Meteor PrintEngine.  
                               // MPS_STARTUP is available for applications to use as an initial status value.
   MPS_FAULT = 0x00000007,   // Obsolete (Was 'Serious fault' on USB hardware)
   MPS_LOADING = 0x00000008,   // Loading print data or print commands from the PC to the PCC
} ePRINTERSTATE;
...
typedef struct {
   ...
   int32   PrintSpeed;   // Fire pulse frequency, Hz
   int32   DocIds[64];   // List of document IDs for documents that have started printing on the NotifyHead HDC since the last status call. See [System] NotifyHead for more information.
   int32   DocCount;   // The number of documents that have started printing on the NotifyHead HDC since the last status call (i.e. number of Ids in the DocIds array)
   int32   PdCount;   // The number of product detects in the current job
   int32   PrintCount;   // The number of printed documents in the current job
   ...
   int32   PrintCount2;   // Lane2 print-count in current job (dual lane mode only).  See <see cref="F:Ttp.Meteor.TAppStatus.PrintCount" /> for more details on when the count increments.
   int32   PrintSpeedJitter;   // Measure of the amount of jitter in the print clock in 0.1% intervals
} TAppStatus;
...
typedef struct {
   ...
   int32   IoSignals;   // I/O signal states for PCC and HDCs 1-6 only.
                        // Bits[31..24] represent PCC GPIO state
                        // Bits[23..20] represent HDC6 GPIO state 
                        // Bits[19..16] represent HDC5 GPIO state 
                        // Bits[15..12] represent HDC4 GPIO state 
                        // Bits[11..8]  represent HDC3 GPIO state 
                        // Bits[7..4]   represent HDC2 GPIO state 
                        // Bits[3..0]   represent HDC1 GPIO state
   int32   bmStatusBits;   // Status word from PCC, BMPS_xxx, see <see cref="T:Ttp.Meteor.Bmps">Bmps</see>
                           // ** <see cref="F:Ttp.Meteor.TAppPccStatus.bmStatusBits" /> is mostly for internal Meteor use and diagnostics, an application should not normally use the bmStatusBits value directly. **
                           // <see cref="F:Ttp.Meteor.TAppPccStatus.bmStatusBits2" /> is available for application use.
   int32   PccState;   // The PCC state <see cref="T:Ttp.Meteor.ePCCSTATE" />.
   ...
   int32   FaultRegister_obsolete;   // Obsolete now. Use TAppHeadStatus::FaultRegister
   int32   AbsXCount;   // Absolute X count (print clocks or pixels). This is the raw encoder value (see AbsXCount) after encoder Multiplier/Divider applied.
                        // It has 24 meaningful bits.
                        // Can be threated as Unsigned 24-bit value, so, wraps around 24 bits E.g  16777215-&gt;0 when increasing or  0-&gt;16777215 when decreasing.
   ...
   int32   EncoderCount;   // Raw Encoder count value, before Multiplier/Divider applied. It has 32 meaningful bits. Not really useful in practice
   ...
   int32   StatusReadTime;   // PrintEngine (PC side) Clock time at which this status was read from the PCC.
                             // As soon as this timestamp is set when receiving status data by PrintEngine, it is pretty useless, because doesn't account for transfer delays from PCC, 
                             // TCP/IP potential buffering etc. FpgaTimestamp_ms field appears to be much more precise.
   int32   IntegFailErr_obsolete;   // Obsolete now. Use TAppHeadStatus::FaultRegister
   eHEADTYPE   HeadType;   // <see cref="T:Ttp.Meteor.eHEADTYPE">Head type</see>
   int32   BitsPerPixel;   // Currently selected bits per pixel
   ...
   int32   TotalPrintRamDwordsPerHdc;   // Total amount of PCC print RAM available per HDC.
                                        // Normally 1GB (1024*1024*1024 bytes) divided by MaxHdcs; can be increased in some configurations using [Test] AllocDramToUsedHdcs = 1.
                                        // This value is the same for every PCC within a subsystem.
} TAppPccStatus;
...
typedef struct {
   ...
   int32   Temperature1;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
   int32   Temperature2;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
   int32   HeadVoltage;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
   ...
   int32   Temperature3;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
   int32   Temperature4;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
   ...
   int32   FaultRegBits;   // HDC Fault Register bits. Low 16 bits is for DataPath A, High 16 bits is for DataPath B
   int32   bmStatusBits3;   // Additional status bits, BMHS3_xxx
   int32   HeaterPwm;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
   int32   AbsHeadVoltage;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
   ...
   int32   Temperature5;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
   int32   Temperature6;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
   int32   Temperature7;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
   int32   Temperature8;   // Meaning depends on a concrete head type. Refer to the corresponding TechNotes.
...
typedef struct {
   ...
   uint32   StatusBits;   // HDC serial number status bits.
                          // BIT30 is set if the requested HDC serial number is not a valid address in the system.
                          // BIT31 is set if the HDC exists, but the HDC type does not support a software serial number.
...
typedef enum {
   TIFF_BPP_ANY = 0x00000001,   // This flag is DEPRECIATED. Use <see cref="F:Ttp.Meteor.bmImageLoadFlags.IMG_LOAD_BPP_ANY" />.
                                // Set: Load the image regardless of the current Meteor bits-per-pixel value, provided the image is at 1,2 or 4 bits-per-pixel
...
typedef struct {
   ...
   uint32   PrintLaneIndex;   // [In] Which print lane to use. 0 or 1 if in dual lane mode, 0 otherwise.
   ...
   char   Content[73];   // [In/Out] The data in the watermark to uniquely identify it, always a string with 72 characters + '\0'.
...
typedef enum {
   ...
   IMG_LOAD_BPP_ANY = 0x00000800,   // Load the image at the file bits-per-pixel value, regardless of the current Meteor bits-per-pixel value. File BPP should be 1,2 or 4.
                                    // If this flag is not set then an attempt to load an image with a bits-per-pixel resolution which does not match Meteor's current setting will fail.
                                    // A one bit per pixel file can however be loaded at 2 or 4 bits per pixel if the <see cref="F:Ttp.Meteor.bmImageLoadFlags.IMG_LOAD_EXPAND" /> 
                                    // flag is set.
} bmImageLoadFlags;
...
typedef struct {
   ...
   int32   CropWidth;   // [In/Out] Width of the cropped image (if IMG_LOAD_CROP is set in Flags). Set to the width used if the image is smaller than the crop.
   int32   CropHeight;   // [In/Out] Height of the cropped image (if IMG_LOAD_CROP is set in Flags). Set to the height used if the image is smaller than the crop.
...
typedef struct {
   ...
   uint32   ScreenModeIdx;   // [IN] If HHR is configured to output 8 BPP or 16 BPP, Meteor uses ScreenPro to screen down to the print BPP.
                             // This selects the screen index to use. Set to 0 to choose the default screen from the config file.
} RipDetails;
...
typedef struct {
   ...
   wchar_t   WidePreviewFilename[MAX_FNAME_LEN];   // [Out] Full path to the generated preview file in utf-16 format. The preview is written to the same directory as the input file.
                                                   // Needs to be decoded using UTF-16 encoding
} RipPreviewDetails;
...
typedef struct {
   ...
   uint32   CtrlFlags;   // Control bit flags.
                         ...
                         // Set Bit2 to exclude *.dmp files from the status report.
                         // Set Bit8 to write the PrintEngine status to the log without creating a status report.  Allows the application to write
                         // snapshots of the status to the log at strategic times (e.g. immediately before an abort), and create the report later.
...
typedef enum {
   ...
   STATUS_DROP_TUNER = 0x00000004,   // Request DropTunerStatus. !! Deprecated !!. Don't use it
   ...
   STATUS_PCC_SERIAL_NUM = 0x0000000F,   // ** DEPRECATED.  WILL BE REMOVED IN A FUTURE RELEASE **.  Request PCC ATSHA serial number.
...
typedef enum {
   SIGEX_PRODUCT_DETECT_QUEUE = 0x000003E8,   // A queue for generating sets of internal product detects at different print intervals is available on Ethernet hardware.
                                              ...
                                              // N.B. The Force Product Detect functionality should not be used at the same time as the Internal Product Detect Queue.
                                              // <para />
                                              // data[0] = SIGEX_PRODUCT_DETECT_QUEUE
                                              // data[1] = Print Engine address: [Bits31:24] PccNum;  [Bits23:16] HdcNum;  [Bits15:0] Unused (should be zero).  
                                              // PccNum must be the index of the master PCC which will generate the Internal Product Detects (normally 1).
                                              // SIGEX_PRODUCT_DETECT_QUEUE should not be sent to slave PCCs ( [ControllerN] Master = 0 in the .cfg file ). 
...
// Parameters sent to the PiUpdateHdcXoffsets function.  The contents are currently unused apart for checking the StructureSizeBytes value.
// Prior to calling PiUpdateHdcXoffsets, the HDC X offset adjustments should be set using <see cref="F:Ttp.Meteor.eCFGPARAMEx.CPEX_OnTheFly_HdcXAdjust_cpix" />.
typedef struct {
   int32   StructureSizeBytes;   // [In] Size of the TUpdateXoffsetParams structure in bytes.  Must be filled in by the user
   int32   Flags;   // Reserved for future used.  Should be set to zero
} TUpdateHdcXoffsetsParams;

// Flag bits used with <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_UPDATE_TRANSLATOR_PARAMS" />
typedef enum {
   bmUpdateTranslator_YOffsets = 0x00000001,   // Update the Y positions of the print-heads. The next translated job will have the new values.
   bmUpdateTranslator_XOffsets = 0x00000002,   // Update the X positions of the print-heads. X offsets will change immediately in the print.
                                               // For 2+ colours per HDC: offsets between colours/heads within the HDC will only update on the next translated job. 
                                               // To keep everything in sync reduce the translate to print delay by keeping the image queue small.
   bmUpdateTranslator_DisabledNozzles = 0x00000004,   // Update the disabled nozzles list. The next translated job will have the new values.
   bmUpdateTranslator_AdjustColourBalance = 0x00000008,   // Update the adjust colour balance values. The next translated job will have the new values.
   bmUpdateTranslator_DroppedNozzles = 0x00000016,   // Update the dropped nozzle values. The next translated job will have the new values.
} UpdateTranslatorFlags;
